<template>
 <div>
   ...
   <group v-for="(item,index) in weather" style="aliign-middle">
     {{item.date}}&nbsp;{{item.fengli}}&nbsp;{{item.type}}&nbsp;{{item.low}}&nbsp;{{item.high}}
   </group>
   <x-button type="primary" @click.native="getWeather">查询天气</x-button>
   <actionsheet :menus="menus" v-model="showMenus" show-cancle></actionsheet>
 </div>
</template>
<script>
import { Divider, Group, XButton, XAddress, XHeader, Actionsheet, ChinaAddressData,
